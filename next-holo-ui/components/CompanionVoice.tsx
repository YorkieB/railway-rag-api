  ...
  return (
    <div className="glass neon-border p-4 md:p-6 space-y-4">
      <div className="flex items-center justify-between gap-3 flex-wrap">
        <h2 className="text-xl font-semibold">AI Companion (Real-Time)</h2>
        <span className="chip">
      ...
      <div className="flex items-center justify-between flex-wrap gap-2">
        {latency.ttft && (
          <div className="text-xs text-slate-200/70">
            <span className={latency.ttft < 800 ? "text-emerald-400" : latency.ttft < 1200 ? "text-amber-400" : "text-red-400"}>
        ...
        {status === "connected" && (
          <button
            ...
            className="text-xs text-slate-200/70 hover:text-white underline"
            type="button"
          >
      ...
      {showMemories && retrievedMemories.length > 0 && (
        <div className="rounded-xl border border-purple-500/30 bg-purple-500/10 p-3 space-y-2">
          <div className="text-xs font-semibold text-purple-300">Retrieved Context:</div>
          {retrievedMemories.map((mem, idx) => (
            <div key={idx} className="text-xs text-slate-200/80 italic border-l-2 border-purple-500/50 pl-2">
              {mem}
            </div>
          ))}
        </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
        ...
        >
          {transcripts.length === 0 && (
            <div className="text-sm text-slate-200/70 italic">Conversation will appear here...</div>
          )}
          ...
          {currentResponse && (
            <motion.div
              initial={{ opacity: 0 }}
              animate={{ opacity: 1 }}
              className="text-sm text-purple-300 bg-purple-500/10 border border-purple-500/20 rounded-lg p-2"
            >
        ...
        <div className="rounded-xl border border-white/10 bg-black/40 p-2 flex flex-col gap-2">
          <div className="text-xs text-slate-300/70 mb-2">Audio Playback</div>
          <div className="flex-1 flex items-center justify-center">
            {status === "connected" ? (
              <div className="text-xs text-emerald-400 animate-pulse">● Streaming</div>
            ) : (
